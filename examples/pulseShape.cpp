...
#define RISEDEF 0.05 //5% rise time used as the t0 point. 10% for seminal Kwong paper
#define UNC 0.35 //uncertainty in definition of rise time
//35% for Run04, but all-natural 0% better for Run03 and Xed
#define LOSTAREA 1.015 //O(1%) adjustment for untraceable tiny bug in S2 areas. But not universal?

using namespace std;

int main ( int argc, char** argv ) { //compile with g++ -Ofast pulseShape.cpp -o pulseShape
  ...
  for ( i = 0; i < num.size(); ++i ) {
    if ( tns[i] >= S2BORD && S2width[num[i]] == 0. ) {
      ...
      if ( soFar > 0.500000*S2tot[num[i]] && CI_right[num[i]]== 0. && CI_left[num[i]] != 0. ) CI_right[num[i]] =tns[i]; //left half only
      if ( CI_left[num[i]] != 0. && CI_right[num[i]] != 0. )
	{ S2width[num[i]] = ( CI_right[num[i]] - CI_left[num[i]] ) / 2.; soFar = 0.0; continue; } //div by 2 as 1s--1s=2sigma
    }
  }
  ...
  cerr << median << "\t" << sigma << "\t" << skew << endl; //can switch to "mean," as desired, but change the header name too :)
