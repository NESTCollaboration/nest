...
paths are already specified, one can simply do:
	...
	```
	cmake -DBUILD_ROOT=ON -DCMAKE_INSTALL_PREFIX=[path to install directory] ../relative/path/NobleElementSimulationTechnique
	```
...
or you do not have the necessary prerequisites.
...
The VDetector files (.cpp, .hh) which serve as the base class for building detectors **should not be edited** 
(except by NEST developers). The VDetector is a virtual detector class inherited by your custom detector.

"DetectorExample_XENON10.hh" is an example of a custom detector file, and is currently set as the default in testNEST.
It serves as a template for users to create their own detector. Follow the steps below (where "MyDetector" is the example).

1. Within the "Detectors/" folder:

	```	
	cp DetectorExample_XENON10.hh MyDetector.hh
	```

2. Then, edit the parameters/functions in MyDetector.hh as desired. 
At minimum, one has to replace every instance of "DetectorExample_XENON10" with "MyDetector": 
	...
	Note that you can add your own custom functions to this detector class.
...
The bareNEST tool is a bare bones, 
...
3. In your simulation's build directory, use whatever cmake command you usually do to build the simulation, but include an additional flag:
