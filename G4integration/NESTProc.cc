...
NESTProc::NESTProc(const G4String& processName, G4ProcessType type, double efield, std::unique_ptr<VDetector> detector)
      : G4VRestDiscreteProcess(processName, type), efield(efield)
{
    
        fNESTcalc = std::unique_ptr<NEST::NESTcalc>(new NEST::NESTcalc(detector.get()));
...
G4VParticleChange* NESTProc::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  auto myLinID = track_lins.find(std::make_tuple(aTrack.GetParentID(),aTrack.GetVertexPosition(),aTrack.GetVertexMomentumDirection()));
  //Type of this step.
...
//  const G4TrackVector* fSecondary = aStep.GetSecondaryInCurrentStep();
  ...
  if(myLinID != track_lins.end() ){
    for (const G4Track* sec : secondaries){
      track_lins.insert(make_pair(make_tuple(sec->GetParentID(),sec->GetPosition(),sec->GetMomentumDirection()),myLinID->second));
    }
  }
  //otherwise, we may need to start a new lineage
    ...
    for (const G4Track* sec : secondaries){
      ...
      if(sec_type!=NoneType)   track_lins.insert(std::make_pair(make_tuple(sec->GetParentID(),sec->GetPosition(),sec->GetMomentumDirection()),lineages.size()-1));
  ...
  //What if the parent is a primary? Give it a lineage just as if it were one of its own secondaries
    if(aTrack.GetParentID()==0){
      ...
      if(sec_type!=NoneType)   track_lins.insert(std::make_pair(make_tuple(aTrack.GetParentID(),aTrack.GetVertexPosition(),aTrack.GetVertexMomentumDirection()),lineages.size()-1));
    }
//  If the current track is part of a lineage...
 
  auto myLinID2 = track_lins.find(make_tuple(aTrack.GetParentID(),aTrack.GetVertexPosition(),aTrack.GetVertexMomentumDirection()));
