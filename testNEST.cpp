...
vector<double> GetS1 ( int Ne );
vector<double> GetS2 ( int Ne );
...
//  double xMin, xMax, yMax, FuncValue;
  ...
  if (argc >= 8) n.SetRandomSeed(atoi(argv[7]));
	...
	if (keV < eMin) keV = eMin;
      ...
      vector<double> scint = GetS1(int(floor(yields.PhotonYield+0.5)));
      printf("%.6f\t%.6f\t%.6f\t", keV, yields.PhotonYield, yields.ElectronYield);
      printf("%.6f\t%.6f\t%.6f\t", scint[2], scint[5], scint[7]);
      scint = GetS2(int(floor(yields.ElectronYield+0.5)));
...
vector<double> GetS1 ( int Nph ) {
  ...
  if ( sPEthr > 0. ) {
    ...
    for ( int i = 0; i < nHits; i++ ) {
      ...
      double phe1 = n.rand_gauss(1.,sPEres) + n.rand_gauss(noise[0],noise[1]); Nphe++;
      prob = n.rand_uniform();
	...
	phe2 = n.rand_gauss(1.,sPEres) + n.rand_gauss(noise[0],noise[1]); Nphe++;
...
vector<double> GetS2 ( int Ne ) {
  ...
  int Nph = int(floor(n.rand_gauss(elYield,sqrt(Fano*elYield))+0.5));
