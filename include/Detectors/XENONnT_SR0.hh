...
public:
  ...
  void Initialization() override {
    ...
    noiseBaseline[0] = 0.0;  // baseline noise mean in PE (Gaussian)				Assumed to be small
    noiseBaseline[1] = 0.0;  // baseline noise width in PE (Gaussian)			 	Assumed to be small
    noiseBaseline[2] = 0.0;  // baseline noise mean in e- (for grid wires)			Assumed to be small
    noiseBaseline[3] = 0.0;  // baseline noise width in e- (for grid wires)			Assumed to be small
    ...
    s2_thr = 20.;      // the S2 threshold in phe or PE, *not* phd. Affects NR most     A conservative cut
    ...
    inGas = false;
    T_Kelvin = 177.15;  // for liquid drift speed calculation				Use 1T's
    p_bar = 1.94;       // gas pressure in units of bars, it controls S2 size		Use 1T's
    ...
    dt_min = 213.19;    // minimum. Top of detector fiducial volume			SR0: 13.346 cm / v_drift
    dt_max = 2140.00;   // maximum. Bottom of detector fiducial volume			SR0: 133.970 cm / v_drift
    dtCntr = 1176.645;  // center of detector for S1 corrections, in usec.		Drift velocity: 0.626 mm/us at 23 V/cm

    radius = 607.3;  // millimeters (fiducial rad)					SR0 fiducial volume cut
    radmax = 664.;   // actual physical geo. limit					TPC radius
    ...
    // In 2-phase, TopDrift=liquid/gas border.
    // nT dimensions are from https://xe1t-wiki.lngs.infn.it/doku.php?id=xenon:xenonnt:analysis:coordinate_system
    TopDrift = 1550.8 ;  // mm not cm or us (but, this *is* where dt=0)			5.0 mm liquid level
    anode = 1553.8;  // the level of the anode grid-wire plane in mm                    3.0 mm above the interface
  ...
  vector<double> FitTBA(double xPos_mm, double yPos_mm,
    ...
    BotTotRat[0] = 0.8;   // S1 bottom-to-total ratio - not that important here, rough estimation
    BotTotRat[1] = 0.252;  // S2 bottom-to-total ratio (important when looking at cS2b!)
