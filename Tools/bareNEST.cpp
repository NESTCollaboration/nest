 ...
 *
 * This skeleton program is meant to demonstrate the bare-bones usage of NEST functions.
 * For the sake of simplicity, this code does not implement any of testNEST's statistical 
 * functionality (bands, energy resolution, etc.). This will generate a SINGLE event.
 */
...
int main ( int argc, char** argv ) {
  ...
  RandomGen::rndm()->SetSeed(0);
	...
	vector<double> vTable, NuisParam={1.,1.}; // NuisParam are scaling factors, for now just for NR Ly & Qy
  ...
  double eMax = 100;
	...
	// NOTE: For heavy nuclei or ions, ion requires specification of "atomNum" and "massNum"
	...
	TestSpectra spec;
	double keV = -999.;
	if (eMin == eMax && eMin >= 0. && eMax > 0. ) keV = eMin;
	else {
		switch (type_num) {
			case CH3T:
			...
			case DD:
				...
				break;
			case WIMP:
				spec.wimp_spectrum_prep = spec.WIMP_prep_spectrum(wimp_mass_GeV, E_step);
  ...
  if ( rho < 1. ) detector->set_inGas(true);
	...
	// and calculate the drift velocity at detector center for normalization purposes
	...
	// time is within bounds (drift time uses the cumulative Z-dependent drift velocity)
	driftTime = -999;
	while (driftTime < detector->get_dt_min() || driftTime > detector->get_dt_max() 
					|| pos_z <= 0 || pos_z > detector->get_TopDrift()) {
  ...
  if ( !MCtruthE ) {
    ...
    if ( usePD == 0 ) Nph = fabs(scint[3]) / (g1*(1.+detector->get_P_dphe()));
    else if ( usePD == 1 ) Nph = fabs(scint[5]) / g1;
    else Nph = fabs(scint[7]) / g1;
    
		if ( usePD == 0 ) Ne = fabs(scint2[5]) / (g2*(1.+detector->get_P_dphe()));
