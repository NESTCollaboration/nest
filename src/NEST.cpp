...
double NESTcalc::PhotonTime(INTERACTION_TYPE species, bool exciton,
    ...
    if (fdetector->get_inGas() || energy < W_DEFAULT * 0.001) {
        ...
        if (fdetector->get_inGas() && !exciton)
            tauR = 28e3;
        else tauR = 0.; //28 microseconds comes from Henrique: https://doi.org/10.1016/j.astropartphys.2018.04.006
...
double NESTcalc::RecombOmegaER(double efield, double elecFrac, const std::vector<double> &FreeParam) {
    ...
    double norm = 1. / (exp(-0.5 * pow(mode - cntr, 2.) / (wide * wide)) *
                        (1. + erf(skew * (mode - cntr) / (wide * sqrt(2.))))); //makes sure omega never exceeds ampl
    double omega = norm * ampl * exp(-0.5 * pow(elecFrac - cntr, 2.) / (wide * wide)) *
                   (1. + erf(skew * (elecFrac - cntr) / (wide * sqrt(2.))));
    if (omega < 0.)
        omega = 0;
    return omega;
}
...
QuantaResult NESTcalc::GetQuanta(const YieldResult &yields, double density,
    ...
    double muCorrection =
            (sqrt(Variance) / widthCorrection) * (skewness / sqrt(1. + skewness * skewness)) * sqrt(2. / M_PI);
...
YieldResult NESTcalc::GetYieldNROld(double energy, int option) { // possible anti-correlation in NR ignored totally
    ...
    if (option ==
        0) { // with old-school L_eff*S_nr conversion, and more explicit Thomas-Imel box model formulae but approximation where Qy can sail off as energy to 0
        FakeField = 1.00;
...
YieldResult NESTcalc::GetYieldIon(double energy, double density, double dfield, double massNum, double atomNum,
    ...
    if (ValidityTests::nearlyEqual(A1, 206.) &&
        ValidityTests::nearlyEqual(Z1, 82.)) { //Pb-206 (from Po-210 alpha decay).
    ...
    if (ValidityTests::nearlyEqual(A1, 206.) && ValidityTests::nearlyEqual(Z1, 82.))
        Ne = RandomGen::rndm()->rand_gauss(Ne / ChargeLoss, 2. * sqrt(Ne / (ChargeLoss *
                                                                            ChargeLoss))); // to compensate for accidentally including Q-loss in fits to Xed data
    ...
    if (ValidityTests::nearlyEqual(Z2, 18.) && ValidityTests::nearlyEqual(Z1, 2.) &&
        ...
        double Qy = (1. / 6200.) *
                    (64478398.7663 - (64478398.7663 * 0.173553719 + (64478398.7663 / 1.21) * (1. - (0.02852 * log(1. +
                                                                                                                  (64478398.7663 /
                                                                                                                   1.21) *
                                                                                                                  ((0.01 /
                                                                                                                    factorE) /
        ...
        double Ly = qu * (1. / 6500.) *
                    (278037.250283 * 0.173553719 + (278037.250283 / 1.21) * (1. - (2. * log(1. +
                                                                                            (278037.250283 / 1.21) *
                                                                                            ((0.653503 / factorE) /
                                                                                             3.))) /
                                                                                  ((278037.250283 / 1.21) *
...
double NESTcalc::GetDriftVelocity_Liquid(double Kelvin, double eField,
    ...
    if (ValidityTests::nearlyEqual(ATOM_NUM,
                                   18.)) { //replace eventually with Kate's work, once T's splined as done for LXe and SXe
