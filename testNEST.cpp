...
int main(int argc, char** argv) {
  ...
  double g2, pos_x, pos_y, pos_z, r, phi, driftTime, field, vD, vD_middle,
      atomNum = 0, massNum = 0, keVee = 0.0, origX, origY;
  ...
  if (verbosity) {
    ...
    cout << "\t\t\t\t\t\t\t\t\t\tNegative numbers are flagging things below "
            "threshold!   phe=(1+P_dphe)*phd & phd=phe/(1+P_dphe)\n";
    ...
    if (type_num == Kr83m && eMin == 9.4 && eMax == 9.4)
      ...
      fprintf(stdout,
              "E [keV]\t\tfield [V/cm]\ttDrift [us]\tX,Y,Z [mm]\tNph\tNe-\tS1 "
              "[PE or phe]\tS1_3Dcor "
              "[phd]\tspikeC(NON-INT)\tNe-Extr\tS2_rawArea [PE]\tS2_3Dcorr "
              "[phd]\n");
  }
  ...
  if (type_num == WIMP) {
    yieldsMax = n.GetYields(
        NR, 25.0, rho, detector->FitEF(0., 0., detector->get_TopDrift() / 2.),
        double(massNum), double(atomNum), NuisParam);
  } else if (type_num == B8) {
    yieldsMax = n.GetYields(
        NR, 4.00, rho, detector->FitEF(0., 0., detector->get_TopDrift() / 2.),
        double(massNum), double(atomNum), NuisParam);
  } else {
    ...
    if (type_num == Kr83m)
      yieldsMax =
          n.GetYields(beta, energyMaximum, rho,
                      detector->FitEF(0., 0., detector->get_TopDrift() / 2.),
                      double(massNum), double(atomNum),
                      NuisParam);  // the reason for this: don't do the special
                                   // Kr stuff when just checking max
    else
      yieldsMax =
          n.GetYields(type_num, energyMaximum, rho,
                      detector->FitEF(0., 0., detector->get_TopDrift() / 2.),
                      double(massNum), double(atomNum), NuisParam);
  }
  ...
  Z_NEW:
    ...
    if (field <= 0.)
      cerr << "\nWARNING: A LITERAL ZERO FIELD MAY YIELD WEIRD RESULTS. USE A "
              "SMALL VALUE INSTEAD.\n";
    if (field > 12.e3)
    ...
    if (type == "muon" || type == "MIP" || type == "LIP" || type == "mu" ||
      ...
      while (zz > 0. &&
             sqrt(pow(xx, 2.) + pow(yy, 2.)) <
                 detector->get_radius()) {  // stop making S1 and S2 if particle
                                            // exits Xe vol
    ...
    if (!MCtruthPos && Nphd_S2 > PHE_MIN) {
      ...
      xySmeared = n.xyResolution(origX, origY, Nphd_S2);
      pos_x = xySmeared[0];
      pos_y = xySmeared[1];
    }
    ...
    vector<double> scint =
        n.GetS1(quanta, pos_x, pos_y, pos_z, vD, vD_middle, type_num, j, field,
    ...
    vector<double> scint2 =
        n.GetS2(quanta.electrons, pos_x, pos_y, driftTime, vD, j, field,
    ...
    if (1) {  // fabs(scint[7]) > PHE_MIN && fabs(scint2[7]) > PHE_MIN ) { //if
      ...
      printf("%.6f\t%.6f\t%.6f\t%.0f, %.0f, %.0f\t%d\t%d\t", keV, field,
             driftTime, pos_x, pos_y, pos_z, quanta.photons,
             quanta.electrons);  // comment this out when below line in
      // printf("%.6f\t%.6f\t%.6f\t%.0f, %.0f,
      // %.0f\t%lf\t%lf\t",keV,field,driftTime,pos_x,pos_y,pos_z,yields.PhotonYield,yields.ElectronYield);
