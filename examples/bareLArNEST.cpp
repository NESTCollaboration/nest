...
int main(int argc, char** argv) 
{
    ...
    std::cerr << "NOTE: This is a skeleton code meant to be a starting point for "
            "custom uses of LArNEST. But you should really look at execNEST."
        << endl
        << endl;
    ...
    NEST::INTERACTION_TYPE type_num = NEST::NR;
    // NEST::INTERACTION_TYPE type_num = NEST::WIMP;
    // NEST::INTERACTION_TYPE type_num = NEST::B8;
    // NEST::INTERACTION_TYPE type_num = NEST::DD;
    // NEST::INTERACTION_TYPE type_num = NEST::AmBe;
    // NEST::INTERACTION_TYPE type_num = NEST::Cf;
    // NOTE: For heavy nuclei or ions, ion requires specification of "atomNum" and
    // "massNum"
    // NEST::INTERACTION_TYPE type_num = NEST::ion;
    // NEST::INTERACTION_TYPE type_num = NEST::gammaRay;
    // NEST::INTERACTION_TYPE type_num = NEST::Kr83m;
    // NEST::INTERACTION_TYPE type_num = NEST::CH3T;
    // NEST::INTERACTION_TYPE type_num = NEST::C14;
    // NEST::INTERACTION_TYPE type_num = NEST::NEST::beta;

    // Do checks for special-case source spectra
    if (type_num == NEST::Kr83m) {
    ...
    } else if (type_num == NEST::gammaRay && verbosity) {
        if (eMin < 10. || eMax < 10.) {
    ...
    if (eMin == eMax && eMin >= 0. && eMax > 0.)
        ...
        //     break;
        // case NEST::B8:  // normalize this to ~3500 / 10-ton / year, for E-threshold of
        //             // 0.5 keVnr, OR 180 evts/t/yr/keV at 1 keV
        //     keV = spec.B8_spectrum(eMin, eMax);
        //     break;
        // case NEST::AmBe:  // for ZEPLIN-III FSR from HA (Pal '98)
    ...
    if (type_num != NEST::WIMP && type_num != NEST::B8) {
