...
YieldResult NESTcalc::GetYields(INTERACTION_TYPE species, double energy,
  ...
  switch (species) {
              ...
              // (TestSpectra)
    {
      ...
      double ThomasImel =
          0.0522 * pow(dfield, -0.0694) * pow(density / 2.9, 0.3);
  ...
  if (Nph > energy / W_SCINT)
    Nph = energy / W_SCINT;  // yields can never exceed 1 / [ W ~ 7 eV ]
...
vector<double> NESTcalc::GetS1(QuantaResult quanta, double dx, double dy,
                               double dz, double driftVelocity, double dV_mid,
                               INTERACTION_TYPE type_num, long evtNum,
                               double dfield, double energy, int useTiming,
                               bool outputTiming, vector<long int>& wf_time,
  ...
  if (useTiming) {
    ...
    photonstream photon_times =
        AddPhotonTransportTime(photon_emission_times, dx, dy, dz);
    ...
    for (ii = 0; ii < (int)fabs(spike); ++ii) {
      ...
      for (int kk = 0; kk < total; ++kk) {
        ...
        AreaTable[index] += 10. * (1. + PULSEHEIGHT) *
  ...
  if (spike >= 1. && spikeC > 0.) {
    newSpike = GetSpike(
        Nph, dx, dy, dz, driftVelocity, dV_mid,
        scintillation);  // over-write spike with smeared version, ~real data
...
vector<double> NESTcalc::GetS2(int Ne, double dx, double dy, double dt,
                               double driftVelocity, long evtNum, double dfield,
  ...
  if (useTiming) {
    ...
    for (i = 0; i < Nee; ++i) {
      ...
      // electronstream[i]*1e+3, newX, newY );
  ...
  double S2b = RandomGen::rndm()->rand_gauss(
      fdetector->get_S2botTotRatio() * pulseArea,
      sqrt(fdetector->get_S2botTotRatio() * pulseArea *
           (1. - fdetector->get_S2botTotRatio())));
  double S2bc = S2b / exp(-dt / fdetector->get_eLife_us()) /
                posDep;  // for detectors using S2 bottom-only in their analyses
...
vector<double> NESTcalc::CalculateG2(bool verbosity) {
  ...
  double epsilonRatio = 1.85 / 1.00126;  // LXe dielectric constant explicitly
                                         // NOT 1.96 (old). Update thx to Dan
                                         // McK.
    ...
    for (zz = pos_z; zz < fdetector->get_TopDrift(); zz += zStep) {
      if (pos_z > fdetector->get_gate()) {
        if (!fdetector->get_inGas())
          ...
          driftTime += zStep / SetDriftVelocity(fdetector->get_T_Kelvin(), rho,
                                                fdetector->get_E_gas() * 1e3);
      } else
        driftTime += zStep / SetDriftVelocity(fdetector->get_T_Kelvin(), rho,
                                              fdetector->FitEF(0., 0., zz));
    }
